...
function decodeLine(line) {
    ...
    const winningNumbers = winningNumberSplit.split(" ").map((v) => {
        return v.trim();
    });
    const myNumbers = lineSplt[1]
        .trim()
        .split(" ")
        .map((v) => {
            return v.trim();
...
function processCardWithCopies(winningNb, myNumbers) {
    ...
    if (duplicates.length !== 0) duplicates = duplicates.slice(1);
...
(async () => {
    ...
    rl.on("line", (line) => {
        ...
        if (winningNbCount !== 0) totalPoints += 2 ** (winningNbCount - 1);
