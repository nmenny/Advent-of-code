...
function isPossibleGame(line) {
    ...
    for (let gameSetId = 0; gameSetId < gameData.length && isPossible; gameSetId++) {
        ...
        for (let cubeDataIdx = 0; cubeDataIdx < cubeData.length && isPossible; cubeDataIdx++) {
            ...
            switch (cubeInfo[1]) {
                case "green":
                    if (cubeInfo[0] > MAX_GREEN) isPossible = false;
                    break;
                case "red":
                    if (cubeInfo[0] > MAX_RED) isPossible = false;
                    break;
                case "blue":
                    if (cubeInfo[0] > MAX_BLUE) isPossible = false;
            }
        }
    }
...
function getPowerOfGame(line) {
    ...
    for (let gameSet of gameData) {
        for (let cubeData of gameSet.trim().split(",")) {
            ...
            if (minNbCubes[color] < coloredCubeCnt) minNbCubes[color] = coloredCubeCnt;
        }
    }

    return Object.values(minNbCubes).reduce((acc, count) => {
        return acc * count;
    });
}
...
(async () => {
    ...
    rl.on("line", (line) => {
        if (isPossibleGame(line)) possibleGamesCount += getGameId(line);
