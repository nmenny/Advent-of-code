...
function toggleLight(lightsMap, lightX, lightY) {
    ...
    if(lightsMap[lightY][lightX] === ON)
        lightsMap[lightY][lightX] = OFF
    else
        lightsMap[lightY][lightX] = ON
}
...
function getNbLightsOn(lightsMap) {
    return lightsMap.map(v => v.reduce((acc, v) => {
...
fs.readFile("./input.txt", (err, data) => {
    ...
    const lightsMap = data.toString().split("\n").map(v => v.split("")).filter(line => line.length > 0);
