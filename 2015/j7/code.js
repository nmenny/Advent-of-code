...
class System {
    ...
    splitInstruction(instruction) {
        return instruction.split(System.affectationInstruction).map((v) => { return v.trim().split(" ").map((v2) => { return v2.trim(); }) })
    }
    ...
    computeInstruction(lopSplitted) {
        ...
        for(let instrId = 0; instrId < System.binaryInstructionSet.length; instrId++) {
            if(lopSplitted.indexOf(System.binaryInstructionSet[instrId]) !== -1) {
                return this.computeBinaryInstruction(System.binaryInstructionSet[instrId], lopSplitted[0], lopSplitted[2]);
            }
        }
...
(async () => {
    ...
    const rl2 = readline.createInterface({ input: fs.createReadStream("./input.txt") });
