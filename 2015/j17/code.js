...
function getNbContainersToHoldGivenQte(containers, targetQte, currQte = 0) {
    ...
    for(const containerQteIdx in containers) {
        const nbAbleToHold = getNbContainersToHoldGivenQte(containers.slice(parseInt(containerQteIdx)+1), targetQte, containers[containerQteIdx] + currQte);
        if(nbAbleToHold !== -1) {
            cnt += nbAbleToHold;
        }
    }
...
function getNbContainersToHoldGivenQteV2(containers, targetQte, currQte = 0, nbContainerUsed=0) {
    if(containers.length === 0) return { cnt: [currQte === targetQte ? 1 : -1], cntUsed: [nbContainerUsed] };

    if(currQte === targetQte) return { cnt: [1], cntUsed: [nbContainerUsed] };

    if(containers.length === 1) return { cnt: [containers[0] + currQte === targetQte ? 1 : -1], cntUsed: [nbContainerUsed+1] };

    let cnt = {cnt : [], cntUsed: []}

    for(const containerQteIdx in containers) {
        const nbAbleToHold = getNbContainersToHoldGivenQteV2(containers.slice(parseInt(containerQteIdx)+1), targetQte, containers[containerQteIdx] + currQte, nbContainerUsed + 1);
